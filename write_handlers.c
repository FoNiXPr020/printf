	...
	int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		...
		{
			if (extra_c)
				buffer[--paded_start] = extra_c;
			return (write(1, &buffer[paded_start], i - paded_start) +
				write(1, &buffer[ind], length - (1 - paded_start)));
		}
	}
	if (extra_c)
		buffer[--ind] = extra_c;
...
/**
 * write_unsigned - Write an unsigned number.
